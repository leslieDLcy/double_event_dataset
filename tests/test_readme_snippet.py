...
for channel_data in get_channel_data(cache_dir, classlabel=classlabel):
    ...
    # 5. get the station inventory for this channel:
    inventory = channel_data.inventory
    ...
    for (trace1, metadata1), (trace2, metadata2) in channel_data.pairs:
        ...
        # or you can remove the response:
        # (https://docs.obspy.org/packages/autogen/obspy.core.trace.Trace.remove_response.html#obspy.core.trace.Trace.remove_response)
        trace1rr = trace1.copy().remove_response(inventory)
        trace2rr = trace1.copy().remove_response(inventory)
