...
In the terminal, Notebook and so on
...
```python
    ...
    classlabel = 'urb_single'  # None means: all labels (see function below)
    ...
    for channel_data in get_events(cache_dir, classlabel=classlabel):
        ...
        # their metadata. So:
        for channel_data in get_events(cache_dir, classlabel='urb_single'):
